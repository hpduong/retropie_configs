...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.chronos Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(chronos,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(chronos,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
################################
				
def SKindex():
    addon_log("SKindex")
    #YN_login = xbmcgui.Dialog().yesno('login','Would you like to enable Adult Conent?')
    #YN_login = koding.YesNo_Dialog(title='login',message='Would you like to enable Adult Conent?')
    #koding.dolog('dialog yesno output %s'%YN_login,line_info=True)
    #if YN_login:
        #from resources.lib.modules import login
        #loggin = login.Loggin().Log_in()
    #if not YN_login:
        #login = False
    if _Edit.facebook_url != '':
        BYB.addDir_file('[COLOR red][I]%s[/COLOR][/I]'%_Edit.facebook_url,'https://www.facebook.com/groups/skydarks/',00,icon,FANART,'','','','')
    if _Edit.twitter_url != 'if you wish ot donate paypal.me/swiperdan':
        BYB.addDir_file('[COLOR lime][I]%s[/COLOR][/I]'%_Edit.twitter_url,'',00,icon,FANART,'','','','')
    #addDir3('[COLOR aqua]TV-Shows[/COLOR]','/',54,'http://images.tenplay.com.au/~/media/TV%20Shows/Movies%20Hub/Movies_Logo_500x281.jpg',FANART,'')
    #addDir3('[COLOR aqua]Movies[/COLOR]','',54,'http://countlessclassics.com/wp-content/uploads/2016/11/Tv-Shows.png',FANART,'')
    getData(_Edit.MainBase,'')
    addDir('[COLOR gold]24/7[/COLOR]','',111,icon,FANART,'','','','')
    addDir('[COLOR gold]Live Sports[/COLOR]','',105,icon,FANART,'','','','')
    #koding.dolog('login = %s'%login,line_info=True)
    #if login:
        # needs changing to suit if you are using getData(_Edit.AdultContent,'') or mode 54
    #addDir('[COLOR red]Adult Content[/COLOR]','',101,icon,FANART,'','','','')
    addDir('[COLOR red]Tools[/COLOR]','',103,icon,FANART,'','','','')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

	
def INDEX350(url):
	open    = OPEN_URL(url) #OPENS THE WEBPAGE AND MAKES IT READABLE
	if 'index' in url:
		part = regex_from_to(open,'<div id="ddmcc_container">','<div style="clear:both;">') # you only have to do this line if there is more than one occurence of what you are looking for, ill show you. we want from <li> to </li> but there are lines with this code in without the movioes. so we just capature the section of the page we want
		all  = regex_get_all(part,'<li>','</li>')
		for match in all:
			url = regex_from_to(match,'href="','"')
			name = regex_from_to(match,'">','<')
			addDir3(name,url,54,icon,fanart,'')
	
	elif '/category/' in url:
		xbmc.log(str(open))
		all  = regex_get_all(open,"<li class='listEpisode'>","</a>")
		for match in all:
			name = regex_from_to(match,'</span>','</a>')
			url  = regex_from_to(match,"a href='","'")
			xbmc.log(str(name))
			xbmc.log(str(url))
			addDir3(str(name).replace("&amp;","&").replace('&#39;',"'").replace('&quot;','"').replace('&#39;',"'").replace('&#8211;',' - ').replace('&#8217;',"'").replace('&#8216;',"'").replace('&#038;','&').replace('&acirc;',''),url,55,icon,fanart,'')
	
	
	elif '/last-movies-added/' in url:
		all  = regex_get_all(open,"<li>","</a> </li>")
		for match in all:
			name = regex_from_to(match,'">','<')
			url  = regex_from_to(match,'a href="','"')
			xbmc.log(str(name))
		...
		all_eps = regex_get_all(open,"<li class='listEpisode'>","</li>") #CAPTURES ALL THE TEXT FROM <li class='listEpisode'> TO </li> EVERYTIME IT ENCOUNTERS THE ENTERED TEXT FROM THE STRING "OPEN"
		for a in all_eps: #IF FOUND, DEFINE THE CAPTURED TEXT AS a
			name = regex_from_to(a,'</span>','</a>') #CAPTURES ALL THE TEXT FROM THE ENTERED START AND STOP POINTS FROM THE STRING a WHICH IS THE EPISODES NAME
			url  = regex_from_to(a,'href="','"') #CAPTURES ALL THE TEXT FROM THE ENTERED START AND STOP POINTS FROM THE STRING a WHICH IS THE EPISODES URL
			date = regex_from_to(a,"'>"," :") #CAPTURES ALL THE TEXT FROM THE ENTERED START AND STOP POINTS FROM THE STRING a WHICH IS THE EPISODES DATE
			addDir3(date+' - '+name,url,55,icon,fanart,'') #ADDS A DIRECTORY OF WITH THE DEFINED STRINGS - 2 BEING THE ACTION THAT IS CALLED WHEN SOMETHING IS CLICKED IN THE CATEGORY
...
def LISTEPS(url):
	...
	for match in all:
		...
		xbmc.log(str(name))
		xbmc.log(str(url))
		addDir3(name,url,56,icon,fanart,'')
		
def LISTLINKS350(url):
	open = OPEN_URL(url)
	all_links = regex_get_all(open,'<td class="odd tdhost">','</td')
	for a in all_links:
		url = regex_from_to(a,'href="','"')
		host= re.compile('href=".*?">(.*?)<',re.DOTALL).findall(a)
		addDir3(str(host).replace("['","").replace("']",""),url,56,icon,fanart,'')
		
def RESOLVE350(url):
	try:
		play = urlresolver.HostedMediaFile(url).resolve()
		if play==False:
			xbmcgui.Dialog().notification('Link is Down','Please Try Another')
...
#this is the fun part lol finding the mistakes.
...
def Search_loop():
    ...
    if Search_name == '':
        ...
        HTML = Open_Url('http://andybuilds79.com/addonstuff/search.txt')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(chronos,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('chronos Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('chronos Searching Please wait',' ')
...
if mode==None:
    ...
    from resources.lib.indexs import tools
    tools.tool_index()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==104:
    from resources.lib.indexs import tools
    tools.del_cache_password()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==105:
    from resources.lib.indexs import livesport
    livesport.livesport_index()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==106:
    from resources.lib.indexs import livesport
    livesport.livesport_scrape_dir()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==107:
    getData(_Edit.LiveSport,'')
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==108:
    from resources.lib.indexs import livesport
    livesport.livesport_scrape_lists(url)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
    ...
    from resources.lib.modules import xbmcplayer
    xbmcplayer.xbmc_player().play(url)
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==111:
    from resources.lib.indexs import twentyfourseven
    twentyfourseven.index()
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
