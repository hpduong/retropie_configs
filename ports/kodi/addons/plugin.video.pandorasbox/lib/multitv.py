...
def multiv_Main_Menu():
    process.Menu('[COLOR slateblue]Search IMDB[/COLOR]','',308,ICON,FANART,'','')
    process.Menu('[COLOR slateblue]TV Schedule[/COLOR]','http://www.tvmaze.com/calendar',6,ICON,FANART,'','')
    process.Menu('[COLOR slateblue]IMDB Top 100 Programs[/COLOR]','http://www.imdb.com/chart/tvmeter?ref_=m_nv_ch_tvm',301,ICON,FANART,'','')
    process.Menu('[COLOR slateblue]Check for episode[/COLOR]','',17,ICON,FANART,'','')
    Pandoras_Box('http://genietvcunts.co.uk/PansBox/ORIGINS/tvcats.php')
