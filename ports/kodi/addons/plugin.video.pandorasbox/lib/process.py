...
ADDON_DATA = USERDATA_PATH + '/plugin.video.pandorasbox/'
...
List = []
watched_list = xbmc.translatePath('special://home/userdata/addon_data/plugin.video.pandorasbox/watched')
...
def Menu(name, url, mode, iconimage, fanart, description, extra, showcontext=True, allinfo={}):
    ...
    if showcontext:
        contextMenu = []
        if showcontext == 'fav':
            contextMenu.append(('Remove from Pandora Favorites', 'XBMC.RunPlugin(%s?mode=12&name=%s)'
                                % (sys.argv[0], urllib.quote_plus(name))))
            contextMenu.append(('Check for episode', 'XBMC.RunPlugin(%s?mode=41&name=%s)'
                                % (sys.argv[0], urllib.quote_plus(name))))								
        if not name in FAV:
            contextMenu.append(('Add to Pandora Favorites',
...
def getFavourites():
    if not os.path.exists(favourites):
        ...
        favList.append(('Pandora Favourites Section', '', '', '', '', '', ''))
        a = open(favourites, "w")
        a.write(json.dumps(favList))
        a.close()
    else:
...
def check_for_episode():
	...
	if not os.path.exists(favourites):
		favList = []
		addon_log('Making Favorites File')
		favList.append(('Pandora Favourite Section', '', '', '', '', '', ''))
		a = open(favourites, "w")
		a.write(json.dumps(favList))
		a.close()
	else:
	...
	if not os.path.exists(watched_list):
		favList = []
		addon_log('Making Favorites File')
		favList.append(('Pandora Watched Section', '', '', '', '', '', ''))
		a = open(watched_list, "w")
		a.write(json.dumps(favList))
		a.close()
	else:
