...
from transistortv_lib import image_proxy
from transistortv_lib import utils2
from transistortv_lib.utils2 import i18n
from transistortv_lib.constants import MODES
from transistortv_lib.db_utils import DB_Connection
from transistortv_lib.trakt_api import Trakt_API
...
class Service(xbmc.Player):
    ...
    def reset(self):
        ...
        self.win.clearProperty('transistortv.playing')
        self.win.clearProperty('transistortv.playing.trakt_id')
        self.win.clearProperty('transistortv.playing.season')
        self.win.clearProperty('transistortv.playing.episode')
        self.win.clearProperty('transistortv.playing.srt')
        self.win.clearProperty('transistortv.playing.trakt_resume')
        self.win.clearProperty('transistortv.playing.transistortv_resume')
        self.win.clearProperty('transistortv.playing.library')
...
def main(argv=None):  # @UnusedVariable
    ...
    transistortv_utils.do_startup_task(MODES.UPDATE_SUBS)
    transistortv_utils.do_startup_task(MODES.PRUNE_CACHE)
    
    while not monitor.abortRequested():
        try:
            is_playing = service.isPlaying()
            transistortv_utils.do_scheduled_task(MODES.UPDATE_SUBS, is_playing)
            transistortv_utils.do_scheduled_task(MODES.PRUNE_CACHE, is_playing)
