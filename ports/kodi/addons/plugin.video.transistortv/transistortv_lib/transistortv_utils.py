    ...
    for cls in relevant_scrapers():
        ...
        if fails >= disable_limit:
            if auto_disable == DISABLE_SETTINGS.ON:
                ...
                ret = dialog.yesno('Transistor TV', line1, line2, line3, utils2.i18n('keep_enabled'), utils2.i18n('disable_it'))
                if ret:
                    kodi.set_setting('%s-enable' % (cls.get_name()), 'false')
                    cur_failures[cls.get_name()] = 0
                else:
...
def is_transistortv():
    ...
    logger.log('Is_transistortv Test (1): |%s|' % (plugin_name))
    if plugin_name == kodi.get_id():
        ...
        logger.log('Is_transistortv Test (2): |%s|' % (path))
        if path:
            try:
                lines = xbmcvfs.File(path).read()
                logger.log('Is_transistortv Test (3): |%s|%s|' % (lines, kodi.get_id()))
                if lines:
