...
if kodi.has_addon('script.transistortv.themepak'):
    themepak_path = xbmcaddon.Addon('script.transistortv.themepak').getAddonInfo('path')
else:
...
def from_playlist():
    ...
    if pl.size() > 0:
        ...
        if li.getfilename().lower().startswith(plugin_url):
            logger.log('Playing Transistor item from playlist |%s|%s|%s|' % (pl.getposition(), li.getfilename(), plugin_url), log_utils.LOGDEBUG)
            return True
