...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.uranus.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.uranus.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1') if condVisibility('System.HasAddon(script.uranus.artwork)') else setting('appearance.alt')
    if appearance == "Uranus": appearance = "Default"
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.uranus.artwork)')
